...
> Asynchronous requests are Apex callouts via an Apex Queueable implementation, `AsyncFunctionInvocationQueueable`.  Queued job runs when system resources become available.  Queueable Apex and Apex callout considerations and limits apply, see [Queueable Apex](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_queueing_jobs.htm).

> Function invocation authentication - who can invoke a function - is managed by a Connected App and Permission Set.  Function invocation authorization - what Organization access a function has - is determined by a Connected App and Session-based Permission Sets. 
...
> An Apex Trigger handles invoking the associated asynchronous callback implementation by enqueuing `FunctionCallbackQueueable`.  Queued job runs when system resources become available.
