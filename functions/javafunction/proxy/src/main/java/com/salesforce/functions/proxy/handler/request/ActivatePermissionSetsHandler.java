...
/**
 * Activate session-based Permission Sets on function's accessToken.
 */
@Order(50)
@Component
public class ActivatePermissionSetsHandler extends BaseRequestHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivatePermissionSetsHandler.class);
    ...
    public void handle(FunctionRequestContext functionRequestContext) throws InvalidRequestException {
        ...
        String activateSessionPermSetUri = proxyConfig.getActivateSessionPermSetUri();
        String url = utils.assembleSalesforceAPIUrl(userContext.getOrgDomainUrl(),
                                                    salesforceContext.getApiVersion(),
                                                    activateSessionPermSetUri);

        // Assemble POST action request
        ResponseEntity<ActionResponse[]> responseEntity = null;
        try {
            String inputJson = utils.toJson(inputs);
            utils.info(LOGGER, requestId, "POST " + activateSessionPermSetUri + ": " + inputJson);
            HttpEntity<String> entity = new HttpEntity<>(inputJson,
                                                         utils.assembleSalesforceAPIHeaders(functionContext.getAccessToken()));
